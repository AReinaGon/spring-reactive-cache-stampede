...
| PostgreSQL         | 15-alpine        | Simulated database (Docker)      |
...
└── reactive-local-caffeine/
    ├── pom.xml
    └── src/
        ├── main/java/com/example/cachestampede/
        ...
        │   ├── repository/TicketRepository.java         # simulated R2DBC source
        │   ├── model/TicketAvailability.java
        │   └── config/CacheConfig.java
        └── test/java/com/example/cachestampede/
            └── StampedeSimulationTest.java
```
...
```bash
# 1. (optional) start the simulated database
docker-compose up -d

# 2. run the stampede simulation test
cd reactive-local-caffeine
mvn test

# 3. or run the app and probe the endpoints
mvn spring-boot:run
curl http://localhost:8080/api/tickets/availability/black-friday-2026
curl http://localhost:8080/actuator/health
```

> The Phase 1 repository is simulated in memory, so the test and the app run without a live
> PostgreSQL instance. Docker is only needed once a real R2DBC repository is introduced in Phase 2.

## What the test proves

`StampedeSimulationTest` fires **2,000 fully concurrent requests** at each service against the same
key and counts how many times the (simulated) database was actually queried:
